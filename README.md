...
- 🌐 **网申页面智能识别 & 网站弹出规则自定义（日常零干扰）**
  - **智能识别招聘网申页**：内置 URL 域名与页面表单语义探测算法，**仅在各大招聘门户、企业校招官网、ATS 系统及网申表单页面中自动显示**。在日常浏览网页（搜索、论坛、视频网站等）时保持完全静默，绝不乱弹气泡或遮挡页面！
  - **当前网站弹出规则一键设置**：点击卡片顶部 🌐 按钮，可随时为当前域名切换：
    - `⚡ 智能检测`（默认遵循网申识别）
    - `✅ 在此网站始终弹出`（加入域名白名单）
    - `🚫 在此网站禁止弹出`（加入域名黑名单，彻底静音）
  - **全局快捷键与图标随时唤醒**：在任何未自动弹出的页面，点击浏览器扩展栏图标或按下 `Alt+C` 快捷键，即可立即就地唤出悬浮卡片！
